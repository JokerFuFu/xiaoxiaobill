...
包含以下路由:
- 综合分析: /api/analysis
- 年度分析: /api/yearly_analysis, /api/yearly_data
- 月度分析: /api/monthly_analysis, /api/monthly_data, /api/filtered_monthly_analysis
- 分类分析: /api/category_analysis, /api/category_expenses, /api/category_detail, /api/category_trend
- 交易记录: /api/transactions, /api/top_transactions
- 时间分析: /api/time_analysis, /api/daily_data
- 概览数据: /api/overview_data, /api/summary
- 辅助接口: /api/categories, /api/available_dates, /api/available_years, /api/category_available_dates
"""
import calendar
...
def _member_analysis_with_colors(df):
    ...
    try:
        ...
        cmap = {m['name']: m.get('color', '#8E8E93') for m in member_svc.list_members(get_current_uid() or 'user_local')}
    except Exception:
        cmap = {}
    for r in rows:
