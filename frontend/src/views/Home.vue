<template>
  <div class="home-page">
    ...
    <!-- AI 主视觉 + 仪表盘 -->
    <template v-else>
      <!-- AI 助手主视觉(首页顶部,AI 原生:居中问候 + 大输入 + 快捷提示) -->
      <section class="ai-hero">
        <router-link to="/ai" class="ai-hero-full" title="打开完整 AI 助手">完整版 <i class="fas fa-up-right-from-square"></i></router-link>
...
/* AI 助手首页主视觉(AI 原生:居中、留白、弱边框,无卡片硬框) */
.ai-hero {
  position: relative;
  max-width: 780px;
...
.ai-hero-full:hover { color: var(--primary-color, #007AFF); background: var(--hover-bg, #f5f5f7); }

@media (max-width: 768px) {
  .ai-hero { padding: 20px 14px; margin-bottom: 16px; }
}
</style>
